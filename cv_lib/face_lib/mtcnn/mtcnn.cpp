...
static std::string strPnet = "../net/Pnet.txt";
static std::string strRnet = "../net/Rnet.txt";
static std::string strOnet = "../net/Onet.txt";

void set_Mtcnn_Net(std::string pnet, std::string rnet, std::string onet) {
	strPnet = pnet;
	strRnet = rnet;
}
...
Pnet::Pnet(){
    ...
    //string filename = "../net/Pnet.txt";
    readData(strPnet, dataNumber, pointTeam);
}
Pnet::~Pnet(){
    freepBox(this->rgb);
    freepBox(this->conv1);
...
Rnet::Rnet(){
    ...
    //string filename = "../net/Rnet.txt";
    readData(strRnet, dataNumber, pointTeam);
...
Onet::Onet(){
    ...
    //string filename = "../net/Onet.txt";
    readData(strOnet, dataNumber, pointTeam);
...
int mtcnn::findFace(Mat &image, Rect &rect, vector<Point2f> &five){
    struct orderScore order;
    int count = 0;
    ...
    for(vector<struct Bbox>::iterator it=thirdBbox_.begin(); it!=thirdBbox_.end();it++){
        if((*it).exist)
		{
			rect.x = (*it).y1;
			rect.y = (*it).x1;
			rect.width = (*it).y2 - (*it).y1;
			rect.height = (*it).x2 - (*it).x1;
            //rectangle(image, Point((*it).y1, (*it).x1), Point((*it).y2, (*it).x2), Scalar(0,0,255), 2,8,0);
			for (int num = 0; num < 5; num++)
			{
				five.push_back(Point2f((float)*(it->ppoint + num), (float)*(it->ppoint + num + 5)));
			}
			break;
