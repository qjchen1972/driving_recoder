...
public:
	...
	const int numPriors = 6;
	const float variance[2] = { 0.1,0.2 };
	const int featureMaps[6] = { 32, 16, 8, 4, 2, 1 };
	const int minSizes[6] = { 26, 51, 95, 138, 182, 225 };
	const int maxSizes[6] = { 51, 95, 138, 182, 225, 269 };
	const int steps[6] = { 8, 16, 32, 64, 128, 256 };
	const int aspectRatios[6][2] = { {2,-1},{2,3}, {2, 3},{2, 3},{2,-1},{2,-1} };
...
#pragma omp parallel for
		for (int i = 0; i < mean.size(); i++) {
			if (mean[i] < 0) mean[i] = 0;			
			else if (mean[i] > 1.0) mean[i] = 1.0;			
		}
...
public:
	...
	void getAnswer(const float *loc_data, const float *conf_data, std::vector<float>  prior_data, std::vector<float> &ans) {
		...
		proConf(conf_data, num_priors, thresHold, idx_class_conf);

		std::vector<std::vector<float> > bboxes;
		decode(loc_data, prior_data, bboxes);

		std::vector<std::pair<std::pair<int, int>, float>> result;
		nms(bboxes, idx_class_conf, nmsThresh, result);
	...
	void nms(std::vector<std::vector<float> > bboxes, std::vector<std::pair<std::pair<int, int>, float>> idx_class_conf,
		...
		for (int i = 0; i < idx_class_conf.size(); i++){
			...
			for (int j = i+1; j < idx_class_conf.size(); j++)
			{
				...
				if (iou_value > nmsthresh)	del[j] = 1;
			}
		}
	}
