...
class ImageReader {
 public:
   ...
   * Ctor and Dtor()
   */
  explicit ImageReader(int32_t width, int32_t height,enum AIMAGE_FORMATS format);
  ...
  */
  ...
  //void RegisterCallback(void* ctx, std::function<void(void* ctx, const char* fileName)>);
   void RegisterCallback(void* ctx, std::function<void(void *ctx, ImageReader *reader)>);

 private:
  int32_t presentRotation_;
  AImageReader* reader_;

  std::function<void(void *ctx, ImageReader *reader)> callback_;
  void *callbackCtx_;
